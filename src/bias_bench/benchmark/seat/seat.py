...
class SEATRunner:
    ...
    def __call__(self):
        ...
        results = []
        for test in tqdm(tests, desc="Running SEAT tests", total=len(tests)):
            ...
            encs_targ1 = _encode(
                self._model, self._tokenizer, encs["targ1"]["examples"], self.device,
            )
            encs_targ2 = _encode(
                self._model, self._tokenizer, encs["targ2"]["examples"], self.device,
            )
            encs_attr1 = _encode(
                self._model, self._tokenizer, encs["attr1"]["examples"], self.device,
            )
            encs_attr2 = _encode(
                self._model, self._tokenizer, encs["attr2"]["examples"], self.device,
            )
...
def _encode(model, tokenizer, texts, device=None):
    encs = {}
    for text in texts:
        # Encode each example.
