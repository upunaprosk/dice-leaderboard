...
def _pretty_model_name(row):
    pretty_name_mapping = {
        ...
        "DistilBertModel": "Distil BERT",
        "DistilRobertaModel": "Distil RoBERTa",
        "QuantizedBertModel": "Quant BERT",
        "QuantizedRobertaModel": "Quant RoBERTa",
    }
...
def _get_baseline_avg_absolute_effect_size(df, model_type):
    model_type_to_baseline = {
        ...
        "pythia": "QuantizedGPTNeoXForCausalLM",
    }
...
if __name__ == "__main__":
    ...
    # Remove pythia extra strs
    df['experiment_id'] = df['experiment_id'].str.replace('EleutherAI_', '')
    df['experiment_id'] = df['experiment_id'].str.replace('EleutherAI-', '')
    df['experiment_id'] = df['experiment_id'].str.replace('pythia_', '')
    df['experiment_id'] = df['experiment_id'].str.replace('pythia-', '')
    df['experiment_id'] = df['experiment_id'].str.replace('-deduped', '')
    df['experiment_id'] = df['experiment_id'].str.replace('-_deduped', '')
    df['experiment_id'] = df['experiment_id'].str.replace('step', '')
    ...
    df["quant_type"] = df["experiment_id"].str.extract(
        r"seat_m-[A-Za-z0-9]+_c-[A-Za-z0-9-\.]+_qt-([A-Za-z-]+)"
    )

    df["quant_prec"] = df["experiment_id"].str.extract(
        r"seat_m-[A-Za-z0-9]+_c-[A-Za-z0-9-\.]+_qt-[A-Za-z-0-9]+_qp-([A-Za-z-0-9]+)"
    )

    if df['quant_type'].isnull().all() == True and df['quant_prec'].isnull().all() == True:
        df["rev"] = df["experiment_id"].str.extract(
            r"seat_m-[A-Za-z0-9]+_c-[A-Za-z0-9-\.]+_rev-([A-Za-z-0-9]+)"
        )
    else:
        df["rev"] = df["experiment_id"].str.extract(
            r"seat_m-[A-Za-z0-9]+_c-[A-Za-z0-9-\.]+_qt-[A-Za-z-0-9]+_qp-[A-Za-z-0-9]+_rev-([A-Za-z-0-9]+)"
        )
    ...
    if df['bias_type'].isna().sum() == df.shape[0]:
        df['bias_type'] = args.bias_type
    ...
    df.to_pickle(f"{args.persistent_dir}/df_pickles/seat_m-{args.model_type}_t-{args.bias_type}.pkl")
