...
declare -A time=(["bert-large-uncased"]="00:20:00" ["roberta-large"]="00:20:00")

for model in ${masked_lm_base_models[@]}; do
    for bias_type in ${all_crows_bias_types[@]}; do
        experiment_id="crows_m-${model}_c-${model_to_largemodel_name_or_path[${model}]}_t-${bias_type}"
        if [ ! -f "${persistent_dir}/results/crows/${experiment_id}.json" ]; then
            echo ${experiment_id}
            sbatch \
                --gres=gpu:nvidia_a100-pcie-40gb:1 \
                --time ${time[${model_to_largemodel_name_or_path[${model}]}]} \
                -J ${experiment_id} \
                ...
                python_job.sh experiments/crows.py \
                    --model ${model} \
                    --model_name_or_path ${model_to_largemodel_name_or_path[${model}]} \
                    --bias_type ${bias_type}
        fi
    done
done
